...
function downloadCalendar() {
  ...
  link.href = url;
  link.download = 'dira-arka-wedding.ics';
...
function validateForm() {
  ...
  fields.forEach((field) => {
    const message = field.input.value.trim() ? '' : field.message;
...
document.querySelectorAll('#calendar-button, #calendar-button-success').forEach((button) => {
...
form.addEventListener('submit', (event) => {
  ...
  document.querySelector('#thank-you-message').textContent = attendance === 'hadir'
    ? 'Terima kasih telah mengonfirmasi. Kehadiran Anda akan membuat hari kami semakin bermakna.'
    : 'Terima kasih sudah memberi kabar. Doa baik Anda tetap kami simpan dekat di hati.';
  form.hidden = true;
  thankYou.hidden = false;
  rsvpSection.querySelector('.rsvp-card__heading').hidden = true;
  thankYou.focus();
});
